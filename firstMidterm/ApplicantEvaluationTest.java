...
class EvaluatorBuilder {
    ...
    private final static Evaluator noCriminalRecord = applicant -> !applicant.hasCriminalRecord();
    private final static Evaluator moreExperience = applicant -> applicant.getEmploymentYears()>=10;
    private final static Evaluator moreCreditScore = applicant -> applicant.getCreditScore()>=500;

    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        switch (type) {
            case NO_CRIMINAL_RECORD: return noCriminalRecord;
            ...
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                return applicant -> noCriminalRecord.evaluate(applicant) && moreCreditScore.evaluate(applicant);
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return applicant -> noCriminalRecord.evaluate(applicant) && moreExperience.evaluate(applicant);
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                return applicant -> moreExperience.evaluate(applicant) && moreCreditScore.evaluate(applicant);
