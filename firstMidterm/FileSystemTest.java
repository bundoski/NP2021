...
// posebna klasa za voveduvanje na \t na fajlovi / folderi
class IndentPrinter {
    public static String printIndentation(int level){
        return IntStream.range(0,level)
                .mapToObj(i -> "\t")
                .collect(Collectors.joining());  // za da gi vrati site naednas pr \t\t\t\t kolku i da ima.
    }
}
...
class File implements IFile{
    ...
    public void sortBySize() {
        return; // nema potreba da sortirame sam fajl..
    }
...
class Folder extends File implements IFile{
    ...
    private boolean ifNameExists(String fileName){
        return files.stream()
                ...
                .anyMatch(name -> name.equals(fileName)); // ako ima match, vrati true, ako ne, vrati false.
    }
