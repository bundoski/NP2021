...
final class IntegerArray {
    ...
    public IntegerArray getSorted() {
        ...
        Arrays.sort(aux);
        return new IntegerArray(aux);
    }

    public IntegerArray concat(IntegerArray ia) {
        int[] aux = Arrays.copyOf(this.arr, arr.length + ia.length());
        for (int i = arr.length; i < arr.length + ia.length(); i++) {
            aux[i] = ia.getElementAt(i - arr.length);
        }
        return new IntegerArray(aux);
    }
